...
the root `tests/conftest.py`'s own scratch-db pattern one level up.
...
same way locally (`docker compose up -d db` inside `backend/`, which
loads the *main* `postwarden` database with schema + both seed files —
untouched by this file, since it only ever creates/uses its own
separate `postwarden_backend_test`) and in CI (`backend-ci.yml`'s
...
ROOT = Path(__file__).resolve().parent.parent.parent  # backend/tests/conftest.py -> repo root
TEST_DB = "postwarden_backend_test"

_base = urlsplit(os.environ.get(
    "DATABASE_URL", "postgresql+psycopg://postwarden:postwarden@localhost:5433/postwarden"
))
