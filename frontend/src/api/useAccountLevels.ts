...
// documents. First caller is `usePostableAccounts.ts` (Phase 3.4), which
// needs `id` -> `depth` to resolve a scenario's own `base_level_id` into
// the depth `fn_line_account_guard` actually posts against — `name`/
// `scenario_count` are real (`account_levels_all`'s own extra column) but
// unused here; Account levels' own CRUD screen (Phase 4.2) is a more
// likely second caller than a reason to trim this down further now.
