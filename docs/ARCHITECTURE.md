...
column, and per-row Edit (rename in place) plus Delete (Tags) or
Archive/Reactivate + Delete (Payees — see the Payees route table row
above for why Delete needed Archive kept alongside it rather than
replacing it). `details.entry-new.quiet` is the "+ Add payee"/"+ Add
tag" bar — `.entry-new`'s own dashed-box/details-summary shape (same as
