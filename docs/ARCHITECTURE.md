...
- **Point-in-time reports** (Trial Balance, Balance Sheet, Variance —
  each has a single "as of" date) use `_shift_date_by_month(date_iso,
  ...
  to an arbitrary day. Rendered the same way Budget Grid's own prev/next
  is: two `<a>` links flanking the filter fields via `justify-content:
  space-between` on the outer `.bar`, labeled with the actual shifted
  date (`&larr; 2026-07-29`).
- **Range reports** (Income Statement, Cash Flow — a `date_from`/
  ...
  screen, and are labeled "Previous/Next period" rather than the actual
  shifted dates, since an arbitrary range can span many months where
  printing real dates inline would read as noise instead of Budget
  Grid's short, scannable single-month label.
