...
| `entries-select.js` | The Journal's "Select entries" mode — toggles a checkbox per entry and the bulk Reverse button (`.select-only` in style.css, hidden until toggled on), same select-all/disabled-until-checked/count-aware-confirm shape as `staging.js`'s Approve/Reject. Alt+R clicks Reverse. |
