...
| Staging | `/staging`, `/staging/approve`, `/staging/reject`, `/staging/{id}/edit`, `/staging/{id}/reject` | review/approve page for whatever's sitting in the one `is_staging` scenario, filterable by the same fields as the Journal (see the row above) — checkboxes + "Approve entries" (Alt+A) copies each into its real target scenario and sets `promoted_entry_id`; the top-of-page **Reject** button is `/staging/reject`'s bulk sibling of the per-entry one, same checkboxes, same permanent delete, looped the same way Approve already is. `pending_staging_entries()` is the shared query the Dashboard's banner count also uses (called with no filter args there). Per-entry **Edit** (a trimmed-down New-entry grid — one fixed target scenario instead of a picker, everything else the same `app.js`/`combobox.js`/`datepicker.js` machinery, see the `app.js` row below) and **Reject** (permanent delete, one entry or many) both only work on a still-pending entry — see SPEC.md decision 15 for why that's even possible given decision 4's append-only rule |
