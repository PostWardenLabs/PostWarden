...
-- 6. Budget scenario: single-sided planning entries (enforce_balance = FALSE)
WITH e AS (
    INSERT INTO journal_entries (scenario_id, entry_date, description)
    VALUES ((SELECT id FROM scenarios WHERE code = 'BUD2026'),
            DATE '2026-08-01', 'August expense budget')
    RETURNING id
)
INSERT INTO journal_lines (entry_id, line_no, account_id, amount, memo)
SELECT e.id, x.line_no, x.account_id, x.amount, x.memo FROM e, (VALUES
    (1, (SELECT id FROM accounts WHERE code = '5210'), 6000.00, 'Groceries target'),
    (2, (SELECT id FROM accounts WHERE code = '5220'), 2500.00, 'Dining out target'),
    (3, (SELECT id FROM accounts WHERE code = '5110'), 9500.00, 'Rent'),
    (4, (SELECT id FROM accounts WHERE code = '5300'), 1800.00, 'Transport target')
) AS x(line_no, account_id, amount, memo);
